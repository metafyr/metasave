...
const NewCard = ({index, handleContactChange, contact}) => {
    ...
    return (
        <div key={index} className='family-card'>
            ...
            <div className='mt-5'>
                ...
                deleteThis ?
                <>
                    <button onClick={() => handleDelete()} className='py-2 bg-green-500 hover:bg-green-800 text-white text-center rounded-bl-[15px] w-full'>Confirm</button>
...
const Family = () => {
  ...
  return (
    <div className='grid my-10'>
        {userDetails && userDetails.contacts.map((user, index) => <Card index={index} user={user} /> )}
        {contacts && contacts.length > 1 && contacts.map((contact, index) => {
            if (index == 0){
                return null
            }
            else {
                return <NewCard contact={contact} handleContactChange={handleContactChange} index={index} />
            }
        })}
